...
template <int Threads, bool VecStats, bool VecOut, bool Indexed>
__global__ __launch_bounds__(Threads, 1) void add_last_layer_norm_f16_small_kernel(
  ...
  const int64_t src = (bidx * T + selected_t) * LN_SMALL_C;
  float sum = 0.0f;
  if constexpr (VecStats) {
#pragma unroll
    for (int k = 0; k < (LN_SMALL_C / 2) / Threads; ++k) {
...
template <int Threads, bool Indexed>
__global__ __launch_bounds__(Threads, 1) void add_last_layer_norm_f16_generic_kernel(
  ...
  const int64_t src = (bidx * T + selected_t) * static_cast<int64_t>(C);
  float sum = 0.0f;
  for (int c = threadIdx.x; c < C; c += Threads) {
...
void launch_add_layer_norm_cmix_mix_f16_stats_cfg(
    const at::Tensor& x,
    const at::Tensor& residual,
    at::Tensor& shift_state,
    const at::Tensor& weight,
  ...
  if (mode == -1) {
    add_layer_norm_cmix_mix_f16_scalar_stats_kernel<LN_SMALL_THREADS>
        <<<static_cast<int>(rows), LN_SMALL_THREADS, 0, stream>>>(
            ...
            x_out.data_ptr<dtype>(), mixed.data_ptr<dtype>(), rows, eps);
  } else if (mode == 0) {
    add_layer_norm_cmix_mix_f16_welford_kernel<false>
        <<<static_cast<int>(rows), 256, 0, stream>>>(
            x.data_ptr<dtype>(), residual.data_ptr<dtype>(), shift_state.data_ptr<dtype>(),
            weight.data_ptr<dtype>(), bias.data_ptr<dtype>(), x_k.data_ptr<dtype>(),
            x_out.data_ptr<dtype>(), mixed.data_ptr<dtype>(), rows, eps);
  } else {
    add_layer_norm_cmix_mix_f16_welford_kernel<true>
        <<<static_cast<int>(rows), 256, 0, stream>>>(
            x.data_ptr<dtype>(), residual.data_ptr<dtype>(), shift_state.data_ptr<dtype>(),
            weight.data_ptr<dtype>(), bias.data_ptr<dtype>(), x_k.data_ptr<dtype>(),
            x_out.data_ptr<dtype>(), mixed.data_ptr<dtype>(), rows, eps);
...
std::vector<at::Tensor> add_layer_norm_cmix_mix_f16_stats_cfg_cuda(
    at::Tensor x,
    at::Tensor residual,
    at::Tensor shift_state,
    at::Tensor weight,
  ...
  const int64_t rows = x.numel() / LN_SMALL_C;
  auto stream = at::cuda::getCurrentCUDAStream();
  launch_add_layer_norm_cmix_mix_f16_stats_cfg(
      x, residual, shift_state, weight, bias, x_k, x_out, mixed, rows,
      static_cast<float>(eps), mode, stream);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  return {x_out, mixed};
}

void add_layer_norm_cmix_mix_f16_stats_cfg_out_cuda(
    at::Tensor x,
    at::Tensor residual,
    at::Tensor shift_state,
    at::Tensor weight,
