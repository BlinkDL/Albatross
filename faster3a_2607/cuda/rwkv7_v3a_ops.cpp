...
torch::Tensor layer_norm_f16_cfg_cuda(torch::Tensor x, torch::Tensor weight, torch::Tensor bias, double eps, int threads, bool vectorized);
...
torch::Tensor linear_f16_orig_lt_cfg_strict_cuda(torch::Tensor x, torch::Tensor weight_orig, int64_t workspace_mb, int64_t algo_index);
torch::Tensor linear_f16_lt_cuda(torch::Tensor x, torch::Tensor weight);
torch::Tensor linear_f16_lt_cfg_cuda(torch::Tensor x, torch::Tensor weight, int64_t workspace_mb, int64_t algo_index);
torch::Tensor linear_f16_lt_cfg_strict_cuda(torch::Tensor x, torch::Tensor weight, int64_t workspace_mb, int64_t algo_index);
...
torch::Tensor linear_f16_m1_splitk_warpred_tile_cuda(torch::Tensor x, torch::Tensor weight, int64_t chunk_k, int64_t tile_cols);
...
std::vector<torch::Tensor> add_layer_norm_f16_cfg_cuda(torch::Tensor x, torch::Tensor residual, torch::Tensor weight, torch::Tensor bias, double eps, int threads, bool vectorized);
void add_layer_norm_f16_cfg_out_cuda(torch::Tensor x, torch::Tensor residual, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_out, torch::Tensor y, double eps, int threads, bool vectorized);
std::vector<torch::Tensor> add_layer_norm_f16_stats_cfg_cuda(torch::Tensor x, torch::Tensor residual, torch::Tensor weight, torch::Tensor bias, double eps, int mode);
void add_layer_norm_f16_stats_cfg_out_cuda(torch::Tensor x, torch::Tensor residual, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_out, torch::Tensor y, double eps, int mode);
...
torch::Tensor add_last_layer_norm_indexed_f16_cuda(torch::Tensor x, torch::Tensor residual, torch::Tensor last_indices, torch::Tensor weight, torch::Tensor bias, double eps);
...
std::vector<torch::Tensor> add_layer_norm_cmix_mix_f16_stats_cfg_cuda(torch::Tensor x, torch::Tensor residual, torch::Tensor shift_state, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_k, double eps, int mode);
void add_layer_norm_cmix_mix_f16_stats_cfg_out_cuda(torch::Tensor x, torch::Tensor residual, torch::Tensor shift_state, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_k, torch::Tensor x_out, torch::Tensor mixed, double eps, int mode);
...
std::vector<torch::Tensor> add_layer_norm_cmix_mix_f16_scalar_stats_cuda(torch::Tensor x, torch::Tensor residual, torch::Tensor shift_state, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_k, double eps);
...
void check_i64_cuda_contig(const torch::Tensor& x, const char* name) {
  TORCH_CHECK(x.is_cuda(), name, " must be CUDA");
  TORCH_CHECK(x.is_contiguous(), name, " must be contiguous");
  TORCH_CHECK(x.scalar_type() == torch::kInt64, name, " must be int64");
}
...
torch::Tensor layer_norm_f16_cfg(
    torch::Tensor x,
    torch::Tensor weight,
    torch::Tensor bias,
    double eps,
...
torch::Tensor linear_orig_rows_f16(torch::Tensor x, torch::Tensor weight_orig, int64_t row_tile, int64_t out_tile) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(weight_orig, "weight_orig");
  TORCH_CHECK(x.dim() >= 2, "x must have at least 2 dims");
  TORCH_CHECK(weight_orig.dim() == 2, "weight_orig must have shape [N, K]");
...
torch::Tensor linear_orig_rows_cfg_f16(torch::Tensor x, torch::Tensor weight_orig, int64_t threads, int64_t row_tile, int64_t out_tile) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(weight_orig, "weight_orig");
  TORCH_CHECK(x.dim() >= 2, "x must have at least 2 dims");
  TORCH_CHECK(weight_orig.dim() == 2, "weight_orig must have shape [N, K]");
...
torch::Tensor linear_f16_orig_lt_cfg(torch::Tensor x, torch::Tensor weight_orig, int64_t workspace_mb, int64_t algo_index) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(weight_orig, "weight_orig");
  TORCH_CHECK(x.dim() >= 2, "x must have at least 2 dims");
  TORCH_CHECK(weight_orig.dim() == 2, "weight_orig must have shape [N, K]");
...
torch::Tensor linear_f16_orig_lt_cfg_strict(torch::Tensor x, torch::Tensor weight_orig, int64_t workspace_mb, int64_t algo_index) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(weight_orig, "weight_orig");
  TORCH_CHECK(x.dim() >= 2, "x must have at least 2 dims");
  TORCH_CHECK(weight_orig.dim() == 2, "weight_orig must have shape [N, K]");
  TORCH_CHECK(x.size(-1) == weight_orig.size(1), "linear_f16_orig_lt_cfg_strict shape mismatch");
  TORCH_CHECK(workspace_mb >= 0 && workspace_mb <= 1024, "workspace_mb out of range");
  TORCH_CHECK(algo_index >= 0 && algo_index < 64, "algo_index out of range");
  return linear_f16_orig_lt_cfg_strict_cuda(x, weight_orig, workspace_mb, algo_index);
}
...
torch::Tensor linear_f16_lt_cfg(torch::Tensor x, torch::Tensor weight, int64_t workspace_mb, int64_t algo_index) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(weight, "weight");
  TORCH_CHECK(x.dim() >= 2, "x must have at least 2 dims");
  TORCH_CHECK(weight.dim() == 2, "weight must have shape [K, N]");
  TORCH_CHECK(x.size(-1) == weight.size(0), "linear_f16_lt_cfg shape mismatch");
  TORCH_CHECK(workspace_mb >= 0 && workspace_mb <= 1024, "workspace_mb out of range");
  TORCH_CHECK(algo_index >= 0 && algo_index < 64, "algo_index out of range");
  return linear_f16_lt_cfg_cuda(x, weight, workspace_mb, algo_index);
}

torch::Tensor linear_f16_lt_cfg_strict(torch::Tensor x, torch::Tensor weight, int64_t workspace_mb, int64_t algo_index) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(weight, "weight");
  TORCH_CHECK(x.dim() >= 2, "x must have at least 2 dims");
  TORCH_CHECK(weight.dim() == 2, "weight must have shape [K, N]");
  TORCH_CHECK(x.size(-1) == weight.size(0), "linear_f16_lt_cfg_strict shape mismatch");
  TORCH_CHECK(workspace_mb >= 0 && workspace_mb <= 1024, "workspace_mb out of range");
  TORCH_CHECK(algo_index >= 0 && algo_index < 64, "algo_index out of range");
  return linear_f16_lt_cfg_strict_cuda(x, weight, workspace_mb, algo_index);
}
...
torch::Tensor linear_f16_m1_splitk_warpred_tile(torch::Tensor x, torch::Tensor weight, int64_t chunk_k, int64_t tile_cols) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(weight, "weight");
  TORCH_CHECK(x.dim() >= 2, "x must have at least 2 dims");
  TORCH_CHECK(weight.dim() == 2, "weight must have shape [K, N]");
...
std::vector<torch::Tensor> add_layer_norm_f16_cfg(
    torch::Tensor x,
...
void add_layer_norm_f16_cfg_out(
    torch::Tensor x,
    torch::Tensor residual,
    torch::Tensor weight,
    torch::Tensor bias,
  ...
  check_half_cuda_contig(weight, "weight");
  check_half_cuda_contig(bias, "bias");
  check_half_cuda_contig(x_out, "x_out");
  check_half_cuda_contig(y, "y");
  ...
  // The CUDA kernel marks every pointer __restrict__; exact-address aliases would make
  // the measurement helper invoke undefined compiler behavior even if writes appear ordered.
  TORCH_CHECK(x_out.data_ptr() != y.data_ptr() &&
                  x_out.data_ptr() != x.data_ptr() && x_out.data_ptr() != residual.data_ptr() &&
                  x_out.data_ptr() != weight.data_ptr() && x_out.data_ptr() != bias.data_ptr() &&
                  y.data_ptr() != x.data_ptr() && y.data_ptr() != residual.data_ptr() &&
                  y.data_ptr() != weight.data_ptr() && y.data_ptr() != bias.data_ptr(),
...
std::vector<torch::Tensor> add_layer_norm_f16_stats_cfg(
    torch::Tensor x,
    torch::Tensor residual,
    torch::Tensor weight,
    torch::Tensor bias,
  ...
  check_half_cuda_contig(weight, "weight");
  check_half_cuda_contig(bias, "bias");
  TORCH_CHECK(x.sizes() == residual.sizes(), "add_layer_norm_f16_stats_cfg x/residual shape mismatch");
  TORCH_CHECK(x.dim() >= 1 && x.size(-1) == 4096, "add_layer_norm_f16_stats_cfg requires C=4096");
  TORCH_CHECK(x.numel() > 0, "add_layer_norm_f16_stats_cfg requires at least one row");
  ...
  TORCH_CHECK(x.get_device() == residual.get_device() && x.get_device() == weight.get_device() &&
                  x.get_device() == bias.get_device(),
              "x, residual, weight, and bias must be on the same CUDA device");
  TORCH_CHECK(x.numel() / 4096 <= INT_MAX, "add_layer_norm_f16_stats_cfg has too many rows");
  check_half2_aligned(x, "x");
  check_half2_aligned(residual, "residual");
  check_half2_aligned(weight, "weight");
  check_half2_aligned(bias, "bias");
  TORCH_CHECK(mode >= 0 && mode <= 2, "mode must be 0 (welford), 1 (welford_cache), or 2 (centered_cache)");
  return add_layer_norm_f16_stats_cfg_cuda(
      x, residual, weight, bias, eps, static_cast<int>(mode));
}

void add_layer_norm_f16_stats_cfg_out(
    torch::Tensor x,
    torch::Tensor residual,
    torch::Tensor weight,
    torch::Tensor bias,
  ...
  check_half_cuda_contig(y, "y");
  TORCH_CHECK(x.sizes() == residual.sizes() && x.sizes() == x_out.sizes() && x.sizes() == y.sizes(),
              "add_layer_norm_f16_stats_cfg_out tensor shape mismatch");
  TORCH_CHECK(x.dim() >= 1 && x.size(-1) == 4096,
              "add_layer_norm_f16_stats_cfg_out requires C=4096");
  TORCH_CHECK(x.numel() > 0, "add_layer_norm_f16_stats_cfg_out requires at least one row");
  TORCH_CHECK(weight.dim() == 1 && weight.size(0) == 4096, "weight shape mismatch");
  TORCH_CHECK(bias.dim() == 1 && bias.size(0) == 4096, "bias shape mismatch");
  TORCH_CHECK(x.get_device() == residual.get_device() && x.get_device() == weight.get_device() &&
                  x.get_device() == bias.get_device() && x.get_device() == x_out.get_device() &&
                  x.get_device() == y.get_device(),
              "all tensors must be on the same CUDA device");
  TORCH_CHECK(x.numel() / 4096 <= INT_MAX, "add_layer_norm_f16_stats_cfg_out has too many rows");
  check_half2_aligned(x, "x");
  check_half2_aligned(residual, "residual");
  check_half2_aligned(weight, "weight");
  check_half2_aligned(bias, "bias");
  check_half2_aligned(x_out, "x_out");
  check_half2_aligned(y, "y");
  // The CUDA pointers are __restrict__. Check byte ranges rather than only
  // equal addresses: shifted contiguous views can partially overlap while
  // exposing different data_ptr() values and would make the kernel undefined.
  check_no_storage_overlap(x_out, "x_out", y, "y");
  check_no_storage_overlap(x_out, "x_out", x, "x");
  check_no_storage_overlap(x_out, "x_out", residual, "residual");
  check_no_storage_overlap(x_out, "x_out", weight, "weight");
  check_no_storage_overlap(x_out, "x_out", bias, "bias");
  check_no_storage_overlap(y, "y", x, "x");
  check_no_storage_overlap(y, "y", residual, "residual");
  check_no_storage_overlap(y, "y", weight, "weight");
  check_no_storage_overlap(y, "y", bias, "bias");
  TORCH_CHECK(mode >= 0 && mode <= 2, "mode must be 0 (welford), 1 (welford_cache), or 2 (centered_cache)");
  add_layer_norm_f16_stats_cfg_out_cuda(
      x, residual, weight, bias, x_out, y, eps, static_cast<int>(mode));
}
...
torch::Tensor add_last_layer_norm_f16(torch::Tensor x, torch::Tensor residual, torch::Tensor weight, torch::Tensor bias, double eps) {
  ...
  TORCH_CHECK(x.size(0) > 0 && x.size(1) > 0, "add_last_layer_norm_f16 requires B,T > 0");
  TORCH_CHECK(x.size(0) <= INT_MAX, "B too large");
  TORCH_CHECK(residual.get_device() == x.get_device() && weight.get_device() == x.get_device() && bias.get_device() == x.get_device(),
              "x, residual, weight, and bias must be on the same CUDA device");
...
torch::Tensor add_last_layer_norm_indexed_f16(
    torch::Tensor x,
    torch::Tensor residual,
    torch::Tensor last_indices,
    torch::Tensor weight,
    torch::Tensor bias,
    double eps) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(residual, "residual");
  check_i64_cuda_contig(last_indices, "last_indices");
  check_half_cuda_contig(weight, "weight");
  check_half_cuda_contig(bias, "bias");
  TORCH_CHECK(x.sizes() == residual.sizes(), "add_last_layer_norm_indexed_f16 x/residual shape mismatch");
  TORCH_CHECK(x.dim() == 3, "x must have shape [B,T,C]");
  const int64_t b = x.size(0);
  TORCH_CHECK(b > 0 && x.size(1) > 0, "add_last_layer_norm_indexed_f16 requires B,T > 0");
  TORCH_CHECK(b <= INT_MAX, "B too large");
  TORCH_CHECK(last_indices.dim() == 1 && last_indices.size(0) == b, "last_indices must have shape [B]");
  TORCH_CHECK(residual.get_device() == x.get_device() && last_indices.get_device() == x.get_device() &&
                  weight.get_device() == x.get_device() && bias.get_device() == x.get_device(),
              "all inputs must be on the same CUDA device");
  const int64_t c = x.size(2);
  TORCH_CHECK(weight.dim() == 1 && weight.size(0) == c, "weight shape mismatch");
  TORCH_CHECK(bias.dim() == 1 && bias.size(0) == c, "bias shape mismatch");
  TORCH_CHECK(c > 0 && c <= 8192, "unsupported C");
  return add_last_layer_norm_indexed_f16_cuda(x, residual, last_indices, weight, bias, eps);
}
...
std::vector<torch::Tensor> add_layer_norm_cmix_mix_f16_scalar_stats(torch::Tensor x, torch::Tensor residual, torch::Tensor shift_state, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_k, double eps) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(residual, "residual");
  check_half_cuda_contig(shift_state, "shift_state");
  check_half_cuda_contig(weight, "weight");
...
void check_add_layer_norm_cmix_mix_f16_stats_inputs(
  ...
  check_half_cuda_contig(bias, "bias");
  check_half_cuda_contig(x_k, "x_k");
  TORCH_CHECK(x.sizes() == residual.sizes(), "fused CMix stats x/residual shape mismatch");
  TORCH_CHECK(x.dim() == 3 && x.size(0) > 0 && x.size(1) == 1 && x.size(2) == 4096,
              "fused CMix stats requires shape [B,1,4096]");
  TORCH_CHECK(x.size(0) <= INT_MAX, "fused CMix stats has too many rows");
  TORCH_CHECK(shift_state.dim() == 2 && shift_state.size(0) == x.size(0) && shift_state.size(1) == 4096,
              "shift_state shape mismatch");
  TORCH_CHECK(weight.dim() == 1 && weight.size(0) == 4096, "weight shape mismatch");
  TORCH_CHECK(bias.dim() == 1 && bias.size(0) == 4096, "bias shape mismatch");
  TORCH_CHECK(x_k.dim() == 1 && x_k.size(0) == 4096, "x_k shape mismatch");
  TORCH_CHECK(x.get_device() == residual.get_device() && x.get_device() == shift_state.get_device() &&
                  x.get_device() == weight.get_device() && x.get_device() == bias.get_device() &&
                  x.get_device() == x_k.get_device(),
              "all fused CMix stats tensors must be on the same CUDA device");
  check_half2_aligned(x, "x");
  check_half2_aligned(residual, "residual");
  ...
  TORCH_CHECK(mode >= -1 && mode <= 1,
              "mode must be -1 (current), 0 (welford), or 1 (welford_cache)");
  // shift_state is read then overwritten through a __restrict__ pointer.
  // Reject shifted views as well as exact aliases with every read-only input.
  check_no_storage_overlap(shift_state, "shift_state", x, "x");
  check_no_storage_overlap(shift_state, "shift_state", residual, "residual");
  check_no_storage_overlap(shift_state, "shift_state", weight, "weight");
  check_no_storage_overlap(shift_state, "shift_state", bias, "bias");
  check_no_storage_overlap(shift_state, "shift_state", x_k, "x_k");
}

std::vector<torch::Tensor> add_layer_norm_cmix_mix_f16_stats_cfg(
    torch::Tensor x,
    torch::Tensor residual,
    torch::Tensor shift_state,
    torch::Tensor weight,
    ...
    double eps,
    int64_t mode) {
  check_add_layer_norm_cmix_mix_f16_stats_inputs(
      x, residual, shift_state, weight, bias, x_k, mode);
  return add_layer_norm_cmix_mix_f16_stats_cfg_cuda(
      x, residual, shift_state, weight, bias, x_k, eps, static_cast<int>(mode));
}

void add_layer_norm_cmix_mix_f16_stats_cfg_out(
    torch::Tensor x,
    torch::Tensor residual,
    torch::Tensor shift_state,
    torch::Tensor weight,
  ...
  check_add_layer_norm_cmix_mix_f16_stats_inputs(
      x, residual, shift_state, weight, bias, x_k, mode);
  check_half_cuda_contig(x_out, "x_out");
  check_half_cuda_contig(mixed, "mixed");
  TORCH_CHECK(x_out.sizes() == x.sizes() && mixed.sizes() == x.sizes(),
              "x_out and mixed must match x shape");
  TORCH_CHECK(x_out.get_device() == x.get_device() && mixed.get_device() == x.get_device(),
              "x_out and mixed must be on the input CUDA device");
  check_half2_aligned(x_out, "x_out");
  check_half2_aligned(mixed, "mixed");
  // All outputs are __restrict__. Byte-range checks catch partially overlapping
  // contiguous views whose data_ptr() values are different.
  check_no_storage_overlap(x_out, "x_out", mixed, "mixed");
  check_no_storage_overlap(x_out, "x_out", shift_state, "shift_state");
  check_no_storage_overlap(mixed, "mixed", shift_state, "shift_state");
  check_no_storage_overlap(x_out, "x_out", x, "x");
  check_no_storage_overlap(x_out, "x_out", residual, "residual");
  check_no_storage_overlap(x_out, "x_out", weight, "weight");
  check_no_storage_overlap(x_out, "x_out", bias, "bias");
  check_no_storage_overlap(x_out, "x_out", x_k, "x_k");
  check_no_storage_overlap(mixed, "mixed", x, "x");
  check_no_storage_overlap(mixed, "mixed", residual, "residual");
  check_no_storage_overlap(mixed, "mixed", weight, "weight");
  check_no_storage_overlap(mixed, "mixed", bias, "bias");
  check_no_storage_overlap(mixed, "mixed", x_k, "x_k");
  add_layer_norm_cmix_mix_f16_stats_cfg_out_cuda(
      x, residual, shift_state, weight, bias, x_k, x_out, mixed, eps, static_cast<int>(mode));
}
...
TORCH_LIBRARY(rwkv7_v3a_ops, m) {
  ...
  m.def("layer_norm_f16_cfg(Tensor x, Tensor weight, Tensor bias, float eps, int threads, bool vectorized) -> Tensor");
  ...
  m.def("linear_f16_orig_lt_cfg_strict(Tensor x, Tensor weight_orig, int workspace_mb, int algo_index) -> Tensor");
  m.def("linear_f16_lt(Tensor x, Tensor weight) -> Tensor");
  m.def("linear_f16_lt_cfg(Tensor x, Tensor weight, int workspace_mb, int algo_index) -> Tensor");
  m.def("linear_f16_lt_cfg_strict(Tensor x, Tensor weight, int workspace_mb, int algo_index) -> Tensor");
  ...
  m.def("add_layer_norm_f16_cfg(Tensor x, Tensor residual, Tensor weight, Tensor bias, float eps, int threads, bool vectorized) -> Tensor[]");
  m.def("add_layer_norm_f16_cfg_out(Tensor x, Tensor residual, Tensor weight, Tensor bias, Tensor(a!) x_out, Tensor(b!) y, float eps, int threads, bool vectorized) -> ()");
  m.def("add_layer_norm_f16_stats_cfg(Tensor x, Tensor residual, Tensor weight, Tensor bias, float eps, int mode) -> Tensor[]");
  m.def("add_layer_norm_f16_stats_cfg_out(Tensor x, Tensor residual, Tensor weight, Tensor bias, Tensor(a!) x_out, Tensor(b!) y, float eps, int mode) -> ()");
  ...
  m.def("add_last_layer_norm_indexed_f16(Tensor x, Tensor residual, Tensor last_indices, Tensor weight, Tensor bias, float eps=" RWKV7_LAYER_NORM_EPS_SCHEMA ") -> Tensor");
  ...
  m.def("add_layer_norm_cmix_mix_f16_stats_cfg(Tensor x, Tensor residual, Tensor(a!) shift_state, Tensor weight, Tensor bias, Tensor x_k, float eps, int mode) -> Tensor[]");
  m.def("add_layer_norm_cmix_mix_f16_stats_cfg_out(Tensor x, Tensor residual, Tensor(a!) shift_state, Tensor weight, Tensor bias, Tensor x_k, Tensor(b!) x_out, Tensor(c!) mixed, float eps, int mode) -> ()");
  m.def("advance_i32(Tensor(a!) x, int amount) -> ()");
}

TORCH_LIBRARY_IMPL(rwkv7_v3a_ops, CUDA, m) {
  m.impl("layer_norm_f16", &layer_norm_f16);
  m.impl("layer_norm_f16_cfg", &layer_norm_f16_cfg);
  ...
  m.impl("linear_f16_orig_lt_cfg_strict", &linear_f16_orig_lt_cfg_strict);
  m.impl("linear_f16_lt", &linear_f16_lt);
  m.impl("linear_f16_lt_cfg", &linear_f16_lt_cfg);
  m.impl("linear_f16_lt_cfg_strict", &linear_f16_lt_cfg_strict);
  ...
  m.impl("linear_f16_m1_splitk_warpred_tile", &linear_f16_m1_splitk_warpred_tile);
  ...
  m.impl("add_layer_norm_f16_cfg", &add_layer_norm_f16_cfg);
  m.impl("add_layer_norm_f16_cfg_out", &add_layer_norm_f16_cfg_out);
  m.impl("add_layer_norm_f16_stats_cfg", &add_layer_norm_f16_stats_cfg);
  m.impl("add_layer_norm_f16_stats_cfg_out", &add_layer_norm_f16_stats_cfg_out);
  m.impl("add_last_layer_norm_f16", &add_last_layer_norm_f16);
  m.impl("add_last_layer_norm_indexed_f16", &add_last_layer_norm_indexed_f16);
  ...
  m.impl("add_layer_norm_tmix_mix6_f16_scalar_stats", &add_layer_norm_tmix_mix6_f16_scalar_stats);
  m.impl("add_layer_norm_cmix_mix_f16_cfg", &add_layer_norm_cmix_mix_f16_cfg);
  m.impl("add_layer_norm_cmix_mix_f16_scalar_stats", &add_layer_norm_cmix_mix_f16_scalar_stats);
  m.impl("add_layer_norm_cmix_mix_f16_stats_cfg", &add_layer_norm_cmix_mix_f16_stats_cfg);
  m.impl("add_layer_norm_cmix_mix_f16_stats_cfg_out", &add_layer_norm_cmix_mix_f16_stats_cfg_out);
